(() => {
  ...
  function fallbackImage(number, name) {
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 372 518"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#0b1933"/><stop offset="1" stop-color="#274a80"/></linearGradient></defs><rect width="372" height="518" rx="22" fill="url(#g)"/><circle cx="186" cy="215" r="72" fill="none" stroke="#55d9f7" stroke-width="8" opacity=".5"/><path d="M112 215h148M186 141v148" stroke="#55d9f7" stroke-width="8" opacity=".35"/><text x="186" y="350" text-anchor="middle" fill="#f3fbff" font-family="system-ui" font-size="28" font-weight="700">${number}</text><text x="186" y="386" text-anchor="middle" fill="#ffc979" font-family="system-ui" font-size="18">${safeName}</text></svg>`;
  ...
  function downloadCollection() {
    const data = JSON.stringify({
      version: 1,
      collection: "ascended-heroes-me2pt5",
  ...
  function loadCollectionFile(file) {
    ...
    reader.addEventListener("load", () => {
      try {
        ...
        if (!data || data.collection !== "ascended-heroes-me2pt5" || typeof data.counts !== "object") throw new Error("Invalid collection");
        counts = data.counts;
        ...
        toast("This is not a valid Ascended Heroes collection.json file.");
      }
    });
  ...
  async function loadData() {
    ...
    if (collectionResponse.ok) {
      ...
      if (collection && collection.collection === "ascended-heroes-me2pt5" && typeof collection.counts === "object") {
  ...
  async function start() {
    ...
    $("#hero-image").src = imageUrl(cards.find((card) => card.number === "295"));
    $("#hero-image").addEventListener("error", (event) => { event.currentTarget.src = fallbackImage("295", "Mega Dragonite ex"); }, { once: true });
